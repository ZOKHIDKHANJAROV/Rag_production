...
try:
    ...
    logging.warning(
        "Reranker model unavailable, fallback ranking will be used. Reason: %s",
        str(e)
    )
...
def rerank_documents(question, docs):
    if reranker is None:
        for d in docs:
            d["rerank_score"] = float(d.get("hybrid_score", d.get("score", 0)))
        docs.sort(
...
async def ask(req: AskRequest):
    ...
    with RAG_LATENCY.time():
        ...
        if not filtered:
            ...
            return {
                "answer": localized_text(lang, "no_info"),
                "sources": []
            }
